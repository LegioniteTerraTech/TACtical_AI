...
namespace TAC_AI.AI
{
    ...
    {
        public class NetworkedNPTInteract : MessageBase
        {
            public NetworkedNPTInteract() { }
            public NetworkedNPTInteract(int playerID, uint techID, int bribeAmount)
            {
                PlayerID = playerID;
                TechID = techID;
        ...
        private static NetworkHook<NetworkedNPTInteract> netHook = new NetworkHook<NetworkedNPTInteract>(OnReceiveNPTRequest, NetMessageType.ToServerOnly);
        ...
        private static void GUIBaseBribe(int lastTeam, int teamFunds)
        {
            if (teamFunds > 0)
            {
            ...
            if (ManBaseTeams.TryGetBaseTeam(lastTeam, out var ETD))
            {
                ...
                else
                {
                    ...
                    if (GUILayout.Button(bribeButton, afford ? AltUI.ButtonGreen : AltUI.ButtonGrey))
                    {
                        if (afford)
                        {
        ...
        { // Bases that store BB
            ...
            GUIBaseBribe(lastTeam, teamFunds);
            GUIBaseAnnoy(lastTeam, teamFunds);
        ...
        public static void TryLoneCommand(NetPlayer player, Tank tech, int bribeAmount)
        {
            if (netHook.CanBroadcast() && !ManNetwork.IsHost)
            {
                if (player)
                    netHook.TryBroadcast(new NetworkedNPTInteract(player.PlayerID, tech.GetTechNetID(), bribeAmount));
            }
            else 
            {
                DoLoneCommand(Singleton.playerTank.visible, tech, bribeAmount);
            }
        }
        private static bool OnReceiveNPTRequest(NetworkedNPTInteract command, bool isServer)
        {
            var player = ManNetwork.inst.GetPlayer(command.PlayerID);
            var techNet = ManNetTechs.inst.FindTech(command.TechID);
            if (player?.CurTech?.tech && techNet)
            {
                if (command.BribeAmount > 0)
                {
                    if (ManPlayer.inst.CanAfford(command.BribeAmount))
                        DoLoneCommand(player.CurTech.tech.visible, techNet.tech, command.BribeAmount);
                }
                else
                {
                    DoLoneCommand(player.CurTech.tech.visible, techNet.tech, command.BribeAmount);
        ...
        private static void DoLoneCommand(Visible provoker, Tank tech, int bribeAmount)
        {
            if (provoker.tank.Team == lastTank.Team)
            {
                var BT = ManBaseTeams.GetTeamAIBaseTeam(provoker.tank.Team);
                lastTank.SetTeam(BT.teamID);
            }
            else if (ManBaseTeams.IsTeammate(lastTank.Team, provoker.tank.Team))
            {
                ...
                else
                {
                    ...
                    ManEnemyWorld.ChangeTeam(lastTank.Team, provoker.tank.Team);
                }
            }
            else
            {
        ...
        private static void GUILoneBribe()
        {
            if (techCost > 0)
            {
                GUILayout.BeginHorizontal(AltUI.TextfieldBlackHuge);
            ...
            if (afford)
            {
                ...
                {
                    ManSFX.inst.PlayUISFX(ManSFX.UISfxType.Buy);
                    TryLoneCommand(ManNetwork.inst.MyPlayer, lastTank, techCost);
                    CloseSubMenuClickable();
                }
            }
            else
        ...
        private static void GUILoneAnnoy()
        {
            if (Singleton.playerTank)
            {
                if (GUILayout.Button(new GUIContent(randomEvict, "Provoke"), AltUI.ButtonRed))
                {
                    ...
                    TryLoneCommand(ManNetwork.inst.MyPlayer, lastTank, 0);
                    CloseSubMenuClickable();
                }
            }
            else if (GUILayout.Button(new GUIContent(randomEvict, "You have no tech!"), AltUI.ButtonGrey))
        ...
        private static void GUILoneTech()
        {
            ...
            else if (ManBaseTeams.IsTeammate(lastTank.Team, playerTeam))
            {
                ...
                {
                    ManSFX.inst.PlayUISFX(ManSFX.UISfxType.Buy);
                    TryLoneCommand(ManNetwork.inst.MyPlayer, lastTank, moneyGive);
                }
            }
            else
            {
